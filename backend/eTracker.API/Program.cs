...
// Add DbContext
// A single configured PostgreSQL context keeps local, Render, and Supabase environments on the same code path.
builder.Services.AddDbContext<ApplicationDbContext>(options =>
    options.UseNpgsql(builder.Configuration.GetConnectionString("DefaultConnection")));

// Add JWT Authentication
// Token validation mirrors the values generated by AuthService so dashboard/API requests stay stateless after login.
...
// Forwarded headers must be applied before auth/redirect logic so Render-originated HTTPS requests are interpreted correctly.
...
// Render health checks hit this lightweight endpoint before any authenticated traffic reaches the API controllers.
